...
export default function StudentsNeedingAttention({
  ...
  const filteredStudents = useMemo(() => {
    return students
      .filter((student) => {
        ...
        if (riskFilter === "HIGH" && student.statusCategory !== "HIGH")
          return false;
        if (riskFilter === "MEDIUM" && student.statusCategory !== "MEDIUM")
          return false;
        if (riskFilter === "HEALTHY" && student.statusCategory !== "HEALTHY")
          return false;
        if (riskFilter === "AT_RISK" && student.statusCategory === "HEALTHY")
          return false;
        ...
        if (searchQuery.trim()) {
          ...
          const matchesSignal = student.signals.some((s) =>
      ...
      .sort((a, b) => {
        ...
        if (sortBy === "quizAsc")
          return a.quizCompletionRate - b.quizCompletionRate;
        return 0;
      });
      ...
      <button
        onClick={(e) => {
          ...
          onReviewStudent
            ...
            : onToggleMonitor && onToggleMonitor(student);
        }}
  ...
  return (
    <section
      ...
      aria-labelledby="students-attention-heading"
    >
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-rose-100 text-rose-700">
                {
                  filteredStudents.filter((s) => s.statusCategory === "HIGH")
                    .length
                }{" "}
                High Priority
              </span>
            ...
            <p className="text-xs sm:text-sm text-slate-500 mt-1">
              Students showing significant changes in learning activity, quiz
              completion, and login frequency.
            </p>
          </div>
                ...
                >
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2.5 min-w-0">
                      <div
                        className={`w-9 h-9 rounded-xl font-bold text-xs flex items-center justify-center shrink-0 shadow-xs ${
                          isHighRisk
      ...
      <div className="p-3.5 sm:p-4 bg-slate-50/80 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-slate-500">
        <span>
          ...
          <strong className="text-slate-800">
            {filteredStudents.length === 0
              ? 0
              : (currentPage - 1) * (pageSize || filteredStudents.length) + 1}
            {pageSize && filteredStudents.length > 0
              ? `-${Math.min(currentPage * pageSize, filteredStudents.length)}`
              : ""}
          </strong>{" "}
          of{" "}
          <strong className="text-slate-800">{filteredStudents.length}</strong>{" "}
          matching students
        </span>
